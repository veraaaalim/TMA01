...
class Book:
    ...
    def borrow(self) -> tuple[bool, str]:
        """
        Decrease available by 1 if > 0. Sanity checks enforced.
        Returns (ok, message)
        """
        if self.available <= 0:
    ...
    def return_one(self) -> tuple[bool, str]:
        """
        Increase available by 1 only if at least one copy was previously borrowed.
        i.e., available < copies, to avoid exceeding stock.
        Returns (ok, message)
        """
        if self.available >= self.copies:
            return False, "Cannot return this title because all copies are already in stock."
        res = self._coll.update_one(
